...
def train_mpscl(model, strain_loader,sval_loader,trgtrain_loader,cfg):
    ...
    mpcl_loss_src = MPCL(num_class=num_classes, temperature=cfg.TRAIN.SRC_TEMP,
                                       base_temperature=cfg.TRAIN.SRC_BASE_TEMP, m=cfg.TRAIN.SRC_MARGIN)

    mpcl_loss_trg = MPCL( num_class=num_classes, temperature=cfg.TRAIN.TRG_TEMP,
    ...
    for i_iter in tqdm(range(cfg.TRAIN.MAX_ITERS+1)):
        ...
        if cfg.TRAIN.MULTI_LEVEL:
            ...
            loss_dice_src_aux     = dice_loss(pred_src_aux, labels_source)
        ...
        loss_dice_src_main     = dice_loss(pred_src_main, labels_source)
...
def train_adaoutput(model, strain_loader,sval_loader, trgtrain_loader, cfg):
    '''
    UDA training
    '''
    # create the model and start the training
    ...
    class_center_feas = torch.zeros((5, 2048), requires_grad=False).cuda()
    best_mean_dice = 0
    for i_iter in tqdm(range(cfg.TRAIN.MAX_ITERS+1)):
        # set train mode for each net
        model.train()
        ...
        if cfg.TRAIN.MULTI_LEVEL:
            pred_trg_aux     = interp(pred_trg_aux)
            d_out_aux        = d_aux(F.softmax(pred_trg_aux,dim=1))
            loss_adv_trg_aux = bce_loss(d_out_aux,source_label)
        else:
            loss_adv_trg_aux = 0
        pred_trg_main     = interp(pred_trg_main)
        d_out_main        = d_main(F.softmax(pred_trg_main,dim=1))
        loss_adv_trg_main = bce_loss(d_out_main,source_label)
        loss = (cfg.TRAIN.LAMBDA_ADV_MAIN * loss_adv_trg_main+
                cfg.TRAIN.LAMBDA_ADV_AUX  * loss_adv_trg_aux) + loss_seg_all
        loss.backward()
        ...
        if cfg.TRAIN.MULTI_LEVEL:
            pred_src_aux = pred_src_aux.detach()
            d_out_aux    = d_aux(F.softmax(pred_src_aux,dim=1))
            loss_d_aux   = bce_loss(d_out_aux,source_label)
            loss_d_aux   = loss_d_aux / 2
            loss_d_aux.backward()
        pred_src_main = pred_src_main.detach()
        d_out_main    = d_main(F.softmax(pred_src_main,dim=1))
        loss_d_main   = bce_loss(d_out_main,source_label)
        loss_d_main   = loss_d_main / 2
        loss_d_main.backward()
...
def train_advent(model, strain_loader,sval_loader, trgtrain_loader, cfg):
    ...
    for i_iter in tqdm(range(cfg.TRAIN.MAX_ITERS+1)):
        ...
        if viz_tensorboard:
            log_losses_tensorboard(writer,current_losses,i_iter)
def train_baseline(model, strain_loader,sval_loader, trgtrain_loader, cfg):
    '''
    baseline training
    '''
    # create the model and start the training
    ...
    for i_iter in tqdm(range(cfg.TRAIN.MAX_ITERS+1)):
        ...
        if viz_tensorboard:
            log_losses_tensorboard(writer,current_losses,i_iter)
def train_proto(model, strain_loader,sval_loader, trgtrain_loader, cfg):
    '''
    UDA training
    '''
    # create the model and start the training
    ...
    optimizer_d_main = optim.Adam(d_main.parameters(), lr=cfg.TRAIN.LEARNING_RATE_D,
                                  betas=(0.9, 0.99))
    # compute class center
    class_center_feas_src = np.load(cfg.TRAIN.CLASS_CENTER_FEA_INIT).squeeze()
    class_center_feas_src = torch.from_numpy(class_center_feas_src).float().cuda()
    class_center_feas_trg = np.load(cfg.TRAIN.CLASS_CENTER_FEA_INIT).squeeze()
    ...
    for i_iter in tqdm(range(cfg.TRAIN.MAX_ITERS+1)):
        ...
        loss =  loss_adv_all + loss_seg_all + class_loss
        loss.backward()
###########################################################################################################
        #Train discriminator networks
        for param in d_aux.parameters():
...
class CriterionMiniBatchCrossImagePair(nn.Module):
    ...
    def forward(self, feat_S, feat_T):
        ...
        sim_dis = sim_dis / (B * B)
        return sim_dis
def train_unbiased_mt(model, ema_model, strain_loader,sval_loader, trgtrain_loader, cfg):
    '''
    UDA training
    '''
    # print("starta")
    if cfg.TRAIN.consistency_type == 'mse':
        consistency_criterion = softmax_mse_loss
    elif cfg.TRAIN.consistency_type == 'kl':
        consistency_criterion = softmax_kl_loss
    else:
        assert False, cfg.TRAIN.consistency_type
    KD_criterion = CriterionMiniBatchCrossImagePair(temperature=1)
    # create the model and start the training
    mseloss = torch.nn.MSELoss()
    input_size_source = cfg.TRAIN.INPUT_SIZE_SOURCE
    ...
    for i_iter in tqdm(range(cfg.TRAIN.MAX_ITERS+1)):
        ...
        ###########################################################################
        # fourier_augmentation(images_source, images_target, "AS", 0.3)
        fourier_aug_target_1 = torch.zeros_like(images_target)
        for img_id in range(images_target.shape[0]):
            index_src = random.randint(0, images_source.shape[0]-1)
            # L_index = random.random()*0.5
            images_source_one = np.transpose(images_source[index_src].data.numpy(), (1, 2, 0))
            images_target_one = np.transpose(images_target[img_id].data.numpy(), (1, 2, 0))
            # aug_images_target = match_histograms(images_target_one,images_source_one)
            _, aug_images_target_1 = fourier_transform(images_source_one, images_target_one, L=0.01, i=1)
            aug_images_target_1 = np.transpose(aug_images_target_1, (2, 0, 1))
            fourier_aug_target_1[img_id,...] = torch.from_numpy(aug_images_target_1).float()

        fourier_aug_target_2 = torch.zeros_like(images_target)
        for img_id in range(images_target.shape[0]):
            index_src = random.randint(0, images_source.shape[0]-1)
            # L_index = random.random()*0.5
            images_source_one = np.transpose(images_source[index_src].data.numpy(), (1, 2, 0))
            images_target_one = np.transpose(images_target[img_id].data.numpy(), (1, 2, 0))
            # aug_images_target = match_histograms(images_target_one,images_source_one)
            _, aug_images_target_2 = fourier_transform(images_source_one, images_target_one, L=0.01, i=1)
            aug_images_target_2 = np.transpose(aug_images_target_2, (2, 0, 1))
            fourier_aug_target_2[img_id,...] = torch.from_numpy(aug_images_target_2).float()

        fourier_aug_source = torch.zeros_like(images_source)
        for img_id_1 in range(images_source.shape[0]):
            index_trg = random.randint(0, images_target.shape[0]-1)
            # L_index = random.random()*0.5
            images_target_one = np.transpose(images_target[index_trg].data.numpy(), (1, 2, 0))
            images_source_one = np.transpose(images_source[img_id_1].data.numpy(), (1, 2, 0))
            # aug_images_target = match_histograms(images_target_one,images_source_one)
            _, aug_images_source = fourier_transform(images_target_one, images_source_one, L=0.01, i=1)
            aug_images_source = np.transpose(aug_images_source, (2, 0, 1))
            fourier_aug_source[img_id_1,...] = torch.from_numpy(aug_images_source).float()
        ###########################################################################
        torch.cuda.empty_cache()
        #############################################for foriner pertrub############################

        feat_score_fourier_target_1, _, score_fourier_target_1_main = model(fourier_aug_target_1.cuda())
        score_fourier_target_1_main = interp(score_fourier_target_1_main)

        feat_score_fourier_target_2, _, score_fourier_target_2_main = model(fourier_aug_target_2.cuda())
        score_fourier_target_2_main = interp(score_fourier_target_2_main)
        with torch.no_grad():
            feat_score_t_aug_curr, _, score_t_aug_curr = ema_model(images_target.cuda())
        score_t_aug_curr = interp(score_t_aug_curr)

        consistency_weight = get_current_consistency_weight(i_iter,cfg=cfg)
        consistency_dist_1 = consistency_criterion(score_fourier_target_1_main, score_t_aug_curr)  # (batch, 2, 112,112,80)
        consistency_dist_1 = torch.mean(consistency_dist_1)

        consistency_dist_2 = consistency_criterion(score_fourier_target_2_main, score_t_aug_curr)  # (batch, 2, 112,112,80)
        consistency_dist_2 = torch.mean(consistency_dist_2)
        consistency_loss = consistency_weight * (consistency_dist_1+consistency_dist_2)

        kd_loss = KD_criterion(feat_score_fourier_target_1, feat_score_t_aug_curr)+KD_criterion(feat_score_fourier_target_2, feat_score_t_aug_curr)
        torch.cuda.empty_cache()
        # # add uncertainty
        # var_l, exp_var_l = cal_variance(score_fourier_target_1_main, score_t_aug_curr)
        # var_2, exp_var_2 = cal_variance(score_fourier_target_2_main, score_t_aug_curr)
        #
        # _, ps_label_2 = torch.max(score_t_aug_curr, dim=1)
        # ps_label_2 = ps_label_2.long()
        # # cps loss
        # cps_loss_1 = torch.mean(exp_var_l * loss_calc(score_fourier_target_1_main, ps_label_2, cfg)) + torch.mean(var_l)+\
        #              torch.mean(exp_var_2 * loss_calc(score_fourier_target_2_main, ps_label_2, cfg)) + torch.mean(var_2)

        _, score_s_aug_curr_aux, score_s_aug_curr = model(fourier_aug_source.cuda())
        if cfg.TRAIN.MULTI_LEVEL:
            score_s_aug_curr_aux     = interp(score_s_aug_curr_aux)
            loss_seg_src_aux_2 = loss_calc(score_s_aug_curr_aux,labels_source,cfg)
            loss_dice_aux_2    = dice_loss(score_s_aug_curr_aux,labels_source)
        else:
            loss_seg_src_aux_2 = 0
            loss_dice_aux_2    = 0
        score_s_aug_curr     = interp(score_s_aug_curr)
        loss_seg_src_main_2 = loss_calc(score_s_aug_curr,labels_source,cfg)
        loss_dice_main_2    = dice_loss(score_s_aug_curr,labels_source)
        loss_seg_all_2 = (cfg.TRAIN.LAMBDA_SEG_SRC_MAIN    * loss_seg_src_main_2
                + cfg.TRAIN.LAMBDA_SEG_SRC_AUX   * loss_seg_src_aux_2
                + cfg.TRAIN.LAMBDA_DICE_SRC_MAIN * loss_dice_main_2
                + cfg.TRAIN.LAMBDA_DICE_SRC_AUX  * loss_dice_aux_2)

        torch.cuda.empty_cache()   ################################清除缓存

        ###########################################################################
        loss_total = loss_seg_all+loss_adv_all+loss_seg_all_2+class_loss+ consistency_loss+kd_loss
                     # +cps_loss_1
        # +loss_infoent+class_loss
        loss_total.backward()
        ...
        optimizer.step()
        update_ema_variables(model, ema_model, cfg.TRAIN.ema_decay, i_iter)
        if cfg.TRAIN.MULTI_LEVEL:
            optimizer_d_aux.step()
        optimizer_d_main.step()
        ####################################
        ...
        current_losses = {'loss_seg_src_aux' :loss_seg_src_aux,
                          ...
                          'loss_d_aux'       :loss_d_aux,
                          'loss_d_main'      :loss_d_main,
                          "loss_seg_2"       :loss_seg_all_2,
                          "loss_kd" :kd_loss,
                          "consistency_dist_1" :consistency_dist_1,
                          "consistency_dist_2": consistency_dist_2,
                          'loss_class'       :class_loss}
        if i_iter % 200 == 0 and i_iter != 0:
            print_losses(current_losses,i_iter)
        if i_iter % 500 == 0 and i_iter != 0:
...
def train_domain_adaptation(model,strain_loader,trgtrain_loader,sval_loader,cfg):
    ...
    if cfg.TRAIN.DA_METHOD == 'baseline':
        print("use method:", cfg.TRAIN.DA_METHOD)
        train_baseline(model, strain_loader, sval_loader,trgtrain_loader, cfg)
    if cfg.TRAIN.DA_METHOD == 'Ours':
        print("use method:", cfg.TRAIN.DA_METHOD)
        train_proto(model, strain_loader, sval_loader,trgtrain_loader, cfg)
